...
- When checking whether a proof is complete, always check both for errors and for commands that might still be processing. If a proof methods runs for more that 3 seconds, consider that a failure and terminate it (by just removing it from the theory).
...
- When you use a proof that was suggested by sledgehammer, leave a special comment 
